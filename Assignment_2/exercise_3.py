...
class MillersModel:
    ...
    def compute_perplexity(self, text):
        ...
        for char in text:
            prob = self.get_prob(char)
            if prob > 0:
                surprise += prob * math.log2(prob)
            else:
                return float('inf')  # If any character has zero probability, perplexity is infinite because entropy is infinite

        entropy = (-1 * surprise) / N
